...
- `deploy_local.ps1` (na raiz do projeto local) - executa os testes da pasta `tests`, cria staging e backup, sincroniza a cópia de produção e reinicia o serviço. Use `-KeepServiceStopped` para instalar uma versão validada sem reativar o monitor. Os testes permanecem apenas no projeto de desenvolvimento; também não são copiados `.env`, `.venv`, `logs`, `data`, `ramais_nomes.json`, `work_calendar.json` ou artefatos locais. Se a validação pós-cópia falhar, o backup é restaurado automaticamente. Após uma publicação bem-sucedida, somente os 5 backups mais recentes são preservados.

Por segurança, o deploy normal exige uma árvore Git limpa. Durante uma publicação intencional ainda não commitada, use `-AllowDirty`; as alterações continuam no projeto do Desktop para revisão e commit posterior. Para manter o projeto pausado durante uma manutenção, use `-KeepServiceStopped`. O script também sincroniza `requirements.lock.txt` no ambiente de produção antes de reiniciar o serviço.
